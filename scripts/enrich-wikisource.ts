...
function sozleriAyikla(wikitext: string) {
  ...
  if (!eslesme) return [];
  return eslesme[1].split(/\n\s*\n+/).map((blok) => blok.split("\n").map(wikiTemizle).filter((satir) => satir && !/^\([^)]*\)$/.test(satir))).filter((satirlar) => satirlar.length >= 2).slice(0, 6).map((satirlar) => ({ tur: "kita" as const, satirlar: satirlar.slice(0, 12) }));
}
...
async function main() {
  ...
  for (const dosya of dosyalar) {
    ...
    if (!metin) { atlanan++; continue; }
    const kategoriler = new Set((sonuc?.parse?.categories ?? []).map((x) => x.category));
    if (!kategoriler.has("Anonim_metinler")) { atlanan++; continue; }
    const sozler = sozleriAyikla(metin);
    if (!sozler.length) { atlanan++; continue; }
    ...
    kayit.sozler = sozler;
    kayit.ozet = `${kayit.yore} yöresine ait, açık arşivde anonim olarak sınıflandırılan geleneksel türkü.`;
    kayit.hikaye = `${kayit.baslik}, ${kayit.yore} yöresinin sözlü müzik belleğinde yer alan geleneksel eserlerden biridir. Vikikaynak kaydı eseri anonim metin olarak sınıflandırır ve söz varyantını açık arşivde korur.\nBu sayfada yalnızca kaynakta doğrulanabilen yöre, anonimlik ve söz bilgileri sunulur. Eserin belirli bir olaya dayanan hikâyesi veya derleme künyesi güvenilir bir kurumsal kaynakla doğrulanana kadar kesin bir anlatı kurulmaz.`;
    kayit.kaynaklar[0].baslik = `Vikikaynak — ${sonuc?.parse?.title ?? kayit.baslik}`;
    kayit.kaynaklar[0].url = `https://tr.wikisource.org/wiki/${encodeURIComponent((sonuc?.parse?.title ?? baslik).replace(/ /g, "_"))}`;

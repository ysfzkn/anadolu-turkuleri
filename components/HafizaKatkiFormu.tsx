...
export function HafizaKatkiFormu({ turkuSlug, varsayilanIl = "" }: { turkuSlug?: string; varsayilanIl?: string }) {
  ...
  async function gonder(e: React.FormEvent) {
    e.preventDefault();
    if (!userId || aciklama.trim().length < 40 || !il.trim()) return;
    ...
    try {
      ...
      const { error } = await supabase.from("hafiza_katkilari").insert({
        ...
        aciklama: aciklama.trim(), dosya_yolu: dosyaYolu,
      });
      if (error) throw error;
  ...
  return <form onSubmit={gonder} className="rounded-3xl border border-toprak/25 bg-white/55 p-5 shadow-motif sm:p-7">
    ...
    <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="max-w-lg text-xs leading-5 text-ceviz-light">Göndererek bu materyali incelememize ve kaynak bilgisiyle arşivde göstermemize izin verdiğini beyan edersin. Telifli kayıt yükleme.</p><button disabled={gonderiliyor || aciklama.trim().length < 40 || !il.trim()} className="min-h-12 shrink-0 rounded-xl bg-kilim px-6 font-semibold text-white disabled:opacity-50">{gonderiliyor ? "Gönderiliyor…" : "Katkıyı gönder"}</button></div>
  </form>;
}

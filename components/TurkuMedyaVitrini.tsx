    ...
    <section aria-labelledby="dinle-izle-baslik" className="mb-8 overflow-hidden rounded-[28px] border border-toprak/30 bg-white/55 shadow-motif">
      ...
      {durum === "yukleniyor" ? (
        <div className="grid gap-4 p-4 sm:p-6 md:grid-cols-2">
          <div className="aspect-[4/3] animate-pulse rounded-2xl bg-toprak/15" />
          <div className="aspect-video animate-pulse rounded-2xl bg-ceviz/10" />
        </div>
      ) : (
        <div className="grid gap-4 p-4 sm:p-6 md:grid-cols-2">
          <article className="overflow-hidden rounded-2xl border border-[#1DB954]/25 bg-[#0f1712] text-white">
            ...
            <div className="flex items-start gap-3 p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                {spotify?.id && !spotify.onizlemeUrl && <p className="mt-1 text-xs text-[#7ee2a0]">Oynatıcıdan tek dokunuşla dinleyebilirsiniz.</p>}
          ...
          <article className="overflow-hidden rounded-2xl border border-[#FF0000]/20 bg-[#151111] text-white">
            <div className="relative aspect-video overflow-hidden bg-black">
              {youtube && youtubeAcik ? (
                ...
                <button type="button" onClick={() => setYoutubeAcik(true)} className="group relative block h-full w-full text-left" aria-label={`${htmlCoz(youtube.baslik)} videosunu oynat`}>
                  <img src={youtube.thumbnail} alt={`${baslik} YouTube video önizlemesi`} className="h-full w-full object-cover opacity-85 transition duration-500 group-hover:scale-[1.025] group-hover:opacity-70" />
                  <span className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-black/10" />
                  <span className="absolute left-1/2 top-1/2 grid h-16 w-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-2xl bg-[#FF0000] shadow-2xl transition group-hover:scale-110"><YouTubeIkon className="h-8 w-8" /></span>
                  <span className="absolute bottom-4 left-4 right-4 text-sm font-semibold text-white">Oynatmak için dokun</span>
                </button>
              ) : (
